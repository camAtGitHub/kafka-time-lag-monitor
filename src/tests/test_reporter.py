...
class TestReporterCycle:
    ...
    def test_output_has_required_fields_and_types(
        ...
        assert consumer["worst_partition"] is None or isinstance(consumer["worst_partition"], int)
...
class TestReporterRunLoop:
    ...
    def test_run_respects_shutdown_event(self, db_conn, mock_config, mock_state_manager):
    ...
    def test_run_updates_thread_last_run(self, db_conn, mock_config, mock_state_manager):
        ...
        mock_state_manager.update_thread_last_run.assert_called_with("reporter", pytest.approx(int(time.time()), abs=5))
