...
import database
from config import Config, KafkaConfig, MonitoringConfig, DatabaseConfig, OutputConfig, ExcludeConfig
import sampler
...
class MockStateManager:
    ...
    def set_group_status(self, group_id: str, topic: str, status: str,
                         status_changed_at: int, last_advancing_at: int,
...
class TestSamplerOnlineGroup:
    ...
    def test_online_group_write_skipped_when_cadence_not_elapsed(self, db_conn):
        ...
        rows = db_conn.execute(
            "SELECT COUNT(*) FROM partition_offsets"
        ).fetchone()[0]
...
class TestSamplerConsumerCommits:
    ...
    def test_consumer_commits_always_written(self, mock_get_committed, db_conn):
        ...
        mock_kafka.get_all_consumed_topic_partitions.return_value = list(topic_partitions.keys())
    ...
    def test_offline_to_recovering_transition(self, db_conn):
        ...
        for i in range(threshold):
            db_conn.execute(
                """INSERT INTO consumer_commits (group_id, topic, partition, committed_offset, recorded_at)
                   VALUES (?, ?, ?, ?, ?)""",
                ("group1", "topic1", 0, 100 + i, current_time - (threshold - i - 1) * 60)
            )
        db_conn.commit()
