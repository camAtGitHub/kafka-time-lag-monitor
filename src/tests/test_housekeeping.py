...
class TestHousekeepingPruning:
    ...
    def test_prune_partition_offsets_keeps_300(
        ...
        # Run housekeeping
        hk = Housekeeping(mock_config, db_conn)
        hk._run_cycle()
    ...
    def test_prune_partition_offsets_keeps_all_if_under_limit(
        ...
        # Insert only 100 rows (under the 300 limit)
        for i in range(100):
            database.insert_partition_offset(
                db_conn, topic, partition, i, now - (100 - i)
            )
        ...
        # Should still have all 100 rows
        cursor = db_conn.execute(
            "SELECT COUNT(*) FROM partition_offsets WHERE topic = ? AND partition = ?",
            (topic, partition)
        )
    ...
    def test_prune_partition_offsets_multiple_topics(
        ...
        for i in range(500):
            database.insert_partition_offset(
                db_conn, "topic1", 0, i, now - (500 - i)
            )
        
        # Insert 50 rows for topic2/partition0
        for i in range(50):
            database.insert_partition_offset(
                db_conn, "topic2", 0, i, now - (50 - i)
            )
...
class TestHousekeepingConsumerCommits:
    ...
    def test_prune_consumer_commits_keeps_all_if_under_limit(
        ...
        # Insert only 50 rows (under the 200 limit)
        for i in range(50):
            database.insert_consumer_commit(
                db_conn, group_id, topic, partition, i, now - (50 - i)
            )
...
class TestHousekeepingVacuum:
    ...
    def test_run_incremental_vacuum_no_error(self, db_conn, mock_config):
...
class TestHousekeepingCycle:
    ...
    def test_cycle_logs_summary(self, db_conn, caplog, mock_config):
        ...
        for i in range(10):
            database.insert_partition_offset(db_conn, "topic1", 0, i, now - i)
            database.insert_consumer_commit(
                db_conn, "group1", "topic1", 0, i, now - i
            )
    ...
    def test_run_respects_shutdown_event(self, db_conn, mock_config):
    ...
    def test_run_raises_exception_does_not_crash(
        self, db_conn, mock_config, caplog, tmp_path
    ):
        """Test that exceptions don't crash the loop."""
        # This test is simplified - we test that the cycle handles 
        # database errors gracefully by testing with invalid config
        # The main loop exception handling is tested indirectly
        
        # Test with an invalid config that will cause issues
        bad_config = type('MockConfig', (), {})()
        bad_config.monitoring = type('MonitoringConfig', (), {})()
        bad_config.monitoring.max_entries_per_partition = -1  # Invalid
        bad_config.monitoring.max_commit_entries_per_partition = -1  # Invalid
        ...
        hk = Housekeeping(bad_config, db_conn)
