...
def load_all_group_statuses(conn: sqlite3.Connection) -> Dict[Tuple[str, str], Dict[str, Any]]:
...
def is_topic_excluded(
    ...
    if topic in config_exclusions:
        return True
    cursor = conn.execute(
        "SELECT 1 FROM excluded_topics WHERE topic = ?",
        (topic,)
    )
...
def is_group_excluded(
    ...
    if group_id in config_exclusions:
        return True
    cursor = conn.execute(
        "SELECT 1 FROM excluded_groups WHERE group_id = ?",
        (group_id,)
    )
    return cursor.fetchone() is not None
...
def get_all_partition_keys(conn: sqlite3.Connection) -> List[Tuple[str, int]]:
    ...
    """
    cursor = conn.execute(
        "SELECT DISTINCT topic, partition FROM partition_offsets"
    )
    return [(row[0], row[1]) for row in cursor.fetchall()]
