...
class Sampler:
        ...
        while not shutdown_event.is_set():
            ...
            try:
                ...
                if not active_groups:
                    ...
                    # Step 2: Get all topic/partitions consumed by those groups
                    topic_partitions = get_all_consumed_topic_partitions(
                    ...
                    # Step 3: Bulk fetch latest produced offsets
                    latest_offsets = {}
                    if topic_partitions:
                        latest_offsets = get_latest_produced_offsets(
                            self._kafka_client, list(topic_partitions)
                        )
                ...
                self._state_manager.update_thread_last_run(
                    "sampler", int(cycle_start)
                )
    ...
    def _process_group(
        self,
        group_id: str,
        all_topic_partitions: Set[Tuple[str, int]],
        ...
        Args:
            ...
            all_topic_partitions: All topic/partitions in the system
